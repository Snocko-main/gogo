...
// setSharedWorkerCountHint is a no-op in stub builds — there is no worker
// pool to size. Defined so RunMultiCore compiles without cgo.
func setSharedWorkerCountHint(int) uint64 { return 0 }

// resetSharedWorkerCountHintIfCurrent is paired with setSharedWorkerCountHint
// in native builds. It is a no-op here because stub builds have no worker pool.
func resetSharedWorkerCountHintIfCurrent(uint64) {}
